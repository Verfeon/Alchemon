...
public partial class BattleManager : Node
{
								...
								Dictionary<string, int> attackerMods, Dictionary<string, int> defenderMods)
	{
		...
		bool isPhysical = IsPhysicalAbility(ability);
	...
	private bool IsPhysicalAbility(Ability ability)
	{
		// Déterminer si une capacité est physique ou spéciale basé sur son type
		// Types physiques : Earth, Fire (certains), Dark (certains)
		// Types spéciaux : Water, Air, Light, Nature, Tech
		
		// Pour simplifier, on peut regarder si le nom contient certains mots-clés
		string name = ability.Name.ToLower();
		
		// Mots-clés physiques
		if (name.Contains("lancer") || name.Contains("frappe") || name.Contains("fouet") || 
			name.Contains("griffe") || name.Contains("coup") || name.Contains("lame"))
		{
			return true;
		}
		
		// Mots-clés spéciaux
		if (name.Contains("rayon") || name.Contains("jet") || name.Contains("flamme") ||
			name.Contains("éclair") || name.Contains("laser") || name.Contains("onde"))
		{
			return false;
		}
		
		// Par défaut, on regarde le type
		return ability.Type == CreatureType.Earth || ability.Type == CreatureType.Fire;
	}
